...
μπαίνοντας στον buffer - έως ότου βρεθεί αλλαγή γραμμής ή κενό έτσι ώστε να "σπάσει" το αρχείο σε λέξεις. Τότε,
ελέγχεται αν το μέγεθος του "mybuffer" είναι μεγαλύτερο από τον αριθμό 7 (http:// - 7 χαρακτήρες) δηλαδή χωράει πιθανώς
κάποιο URL - το οποίο και ελέγχεται (αν ξεκινάει με http:// τότε έχουμε URL). Έπειτα, με την ίδια λογική ελέγχεται αν
υπάρχει το 'www.' ώστε να αφαιρεθεί, θέτοντας κατάλληλα την αρχή του for loop (για να αγνοηθούν οι θέσεις που κρατούν
τους συγκεκριμένους χαρακτήρες). Στο string "loc" διαμορφώνεται το location, αφού γνωρίζουμε πως το κενό, η αλλαγή
γραμμής και το slash σηματοδοτούν το τέλος του. Αφού επιτέλους πάρουμε το "loc", ελέγχουμε αν υπάρχει μέσα στο map με
όνομα "locations". Αν ναι, τότε αυξάνεται ο μετρητής κατά ένα, διαφορετικά εισάγεται ως νέο κλειδί με τιμή 1.
